...
// 定义费用标准
const COST_PER_MIL_INPUT = apiCost.input; // 每百万输入 token 的费用
const COST_PER_MIL_OUTPUT = apiCost.output; // 每百万输出 token 的费用
// 重试次数
const MAX_RETRIES = 3;
...
const readSrtFile = async (filePath) => {
  try {
    ...
    console.error(chalk.red('读取 SRT 文件失败:'), error);
    throw error;
  }
};
...
const translateSubtitles = async (subtitles, previousTranslation = '', maxTokens, apiKey) => {
  ...
  while (attempts < MAX_RETRIES) {
    try {
      ...
      console.log(chalk.blue(`输入 Tokens: ${inputTokens}`), chalk.green(`输出 Tokens: ${outputTokens}`), chalk.yellow(`本次花费: $${totalCost.toFixed(4)}`));
...
const main = async () => {
  ...
  try {
    ...
    console.log(chalk.green(`总花费: $${totalCost.toFixed(4)}`)); // 输出总花费，保留四位小数
  } catch (error) {
