...
const MAX_TOKENS = 100; // 设置每次请求的最大token数, 常规内容设置为 3000, 敏感内容设置为 100
...
// 翻译字幕
const translateSubtitles = async (subtitles, previousTranslation = '') => {
    let attempts = 0;
...
const splitSubtitles = (subtitles) => {
    ...
    for (const line of lines) {
        ...
        if ((countTokens(currentPart) + tokens) > MAX_TOKENS) {
...
const main = async () => {
    ...
    try {
        ...
        for (let i = 0; i < totalParts; i++) {
            ...
            const translatedPart = await translateSubtitles(part, previousTranslation);
            if (translatedPart) {
